...
void vzm::helpers::ComputePosSS2WS(const float x, const float y, const float d, const VID camera, const VID renderer, float posWS[3])
{
    ...
    float x_ndc = 2.0f * x / (float) vp.width - 1.0f;
    float y_ndc = 1.0f - 2.0f * y / (float) vp.height;
...
void vzm::helpers::ComputePosSS2CS(const float x, const float y, const float d, const VID camera, const VID renderer, float posCS[3])
{
    VzRenderPath* render_path = gEngineApp->GetRenderPath(renderer);
    if (render_path == nullptr) return;
    const filament::Viewport& vp = render_path->GetView()->getViewport();
