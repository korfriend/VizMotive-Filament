...
const uint32_t VzCube::mIndices[] = {
    // solid
    2,0,1, 2,1,3,  // far
    6,4,5, 6,5,7,  // near
    2,0,4, 2,4,6,  // left
...
const filament::math::float3 VzCube::mVertices[] = {
...
VzCube::VzCube(Engine& engine, filament::Material const* material, float3 linearColor, bool culling) :
    mEngine(engine),
    mMaterial(material) {
...
void VzCube::mapFrustum(filament::Engine& engine, Camera const* camera) {
...
void VzCube::mapFrustum(filament::Engine& engine, filament::math::mat4 const& transform) {
    // the Camera far plane is at infinity, but we want it closer for display
    mat4f p(transform);
...
void VzCube::mapAabb(filament::Engine& engine, filament::Box const& box) {
