...
VzIBL::VzIBL(Engine& engine) : mEngine(engine) {
}
...
bool VzIBL::loadFromEquirect(Path const& path) {
    if (!path.exists()) {
        return false;
    }
...
bool VzIBL::loadFromKtx(const std::string& prefix) {
...
bool VzIBL::loadFromDirectory(const utils::Path& path) {
...
bool VzIBL::loadCubemapLevel(filament::Texture** texture, const utils::Path& path, size_t level,
