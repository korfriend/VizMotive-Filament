...
std::tuple<VkImage, VkDeviceMemory> createImageAndMemory(
    VulkanContext const& context, VkDevice device, VkExtent2D extent,
    VkFormat format) {
  bool const isDepth = isVkDepthFormat(format);
  // Filament expects blit() to work with any texture, so we almost always set
  // these usage flags.
  ...
  VkImageCreateInfo imageInfo{
      ...
      .usage =
          blittable | (isDepth ? VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT
  ...
  uint32_t memoryTypeIndex = context.selectMemoryType(
      memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

  FILAMENT_CHECK_POSTCONDITION(memoryTypeIndex < VK_MAX_MEMORY_TYPES)
      << "VulkanPlatformSwapChainImpl: unable to find a memory type that meets "
         "requirements.";
...
std::tuple<VkImage, VkDeviceMemory, void * UTILS_NULLABLE> createImageAndMemoryHeadless(
  ...
  // Add external memory creation info
...
VkFormat selectDepthFormat(VkFormatList const& depthFormats, bool hasStencil) {
  auto const formatItr =
      std::find_if(depthFormats.begin(), depthFormats.end(),
                   hasStencil ? isVkStencilFormat : isVkDepthFormat);
  assert_invariant(formatItr != depthFormats.end() &&
                   "Cannot find suitable swapchain depth format");
  return *formatItr;
}
...
VulkanPlatformSwapChainImpl::~VulkanPlatformSwapChainImpl() { destroy(); }
...
void VulkanPlatformSwapChainImpl::destroy() {
  ...
  // Note: Hardware-backed swapchain images are not owned by us and should not
  // be destroyed.
  mSwapChainBundle.colors.clear();
}

VkImage VulkanPlatformSwapChainImpl::createImage(VkExtent2D extent,
                                                 VkFormat format) {
  auto [image, memory] =
      createImageAndMemory(mContext, mDevice, extent, format);
  mMemory.insert({image, memory});
  return image;
}

VulkanPlatformSurfaceSwapChain::VulkanPlatformSurfaceSwapChain(
    VulkanContext const& context, VkPhysicalDevice physicalDevice,
...
VkResult VulkanPlatformSurfaceSwapChain::create() {
  ...
  if (maxImageCount != 0 && desiredImageCount > maxImageCount) {
    FVK_LOGE << "Swap chain does not support " << desiredImageCount
             << " images." << utils::io::endl;
    desiredImageCount = caps.minImageCount;
  }

  // Find a suitable surface format.
  FixedCapacityVector<VkSurfaceFormatKHR> const surfaceFormats = enumerate(
      vkGetPhysicalDeviceSurfaceFormatsKHR, mPhysicalDevice, mSurface);
  ...
  // Verify that our chosen present mode is supported. In practice all devices
  // support the FIFO mode, but we check for it anyway for completeness.  (and
  // to avoid validation warnings)
  VkPresentModeKHR const desiredPresentMode = VK_PRESENT_MODE_FIFO_KHR;
  FixedCapacityVector<VkPresentModeKHR> presentModes = enumerate(
  ...
  VkSwapchainCreateInfoKHR const createInfo{
      ...
      .imageUsage =
          ...
          | VK_IMAGE_USAGE_TRANSFER_SRC_BIT,  // Allows use as a blit source
                                              // (for readPixels)
      ...
      // more efficient to handle this ourselves by setting this field to be
      // equal to the currentTransform mask in the caps, but this would involve
  ...
  VkResult result =
      vkCreateSwapchainKHR(mDevice, &createInfo, VKALLOC, &mSwapchain);
  FILAMENT_CHECK_POSTCONDITION(result == VK_SUCCESS)
      << "vkGetPhysicalDeviceSurfaceFormatsKHR error: "
      << static_cast<int32_t>(result);

  mSwapChainBundle.colors =
      enumerate(vkGetSwapchainImagesKHR, mDevice, mSwapchain);
  ...
  mSwapChainBundle.depth =
      createImage(mSwapChainBundle.extent, mSwapChainBundle.depthFormat);

  FVK_LOGI << "vkCreateSwapchain"
           << ": " << mSwapChainBundle.extent.width << "x"
           << mSwapChainBundle.extent.height << ", " << surfaceFormat.format
           << ", " << surfaceFormat.colorSpace << ", "
           << "swapchain-size=" << mSwapChainBundle.colors.size() << ", "
           << "identity-transform=" << (caps.currentTransform == 1) << ", "
           << "depth=" << mSwapChainBundle.depthFormat << io::endl;
...
VkResult VulkanPlatformSurfaceSwapChain::acquire(
    VulkanPlatform::ImageSyncData* outImageSyncData) {
  mCurrentImageReadyIndex =
      (mCurrentImageReadyIndex + 1) % IMAGE_READY_SEMAPHORE_COUNT;
  ...
  VkResult result = vkAcquireNextImageKHR(
      ...
      VK_NULL_HANDLE, &outImageSyncData->imageIndex);
...
VkResult VulkanPlatformSurfaceSwapChain::present(uint32_t index,
  ...
  // On Android Q and above, a suboptimal surface is always reported after
  // screen rotation:
  // https://android-developers.googleblog.com/2020/02/handling-device-orientation-efficiently.html
  if (result == VK_SUBOPTIMAL_KHR && !mSuboptimal) {
...
VulkanPlatformHeadlessSwapChain::VulkanPlatformHeadlessSwapChain(
    ...
    : VulkanPlatformSwapChainImpl(context, device, queue), mCurrentIndex(0) {
  mSwapChainBundle.extent = extent;
  mSwapChainBundle.colorFormat =
      (flags & backend::SWAP_CHAIN_CONFIG_SRGB_COLORSPACE) != 0
          ? VK_FORMAT_R8G8B8A8_SRGB
          : VK_FORMAT_R8G8B8A8_UNORM;
  ...
  for (size_t i = 0; i < HEADLESS_SWAPCHAIN_SIZE; ++i) {
    auto [image, memory, handle] = createImageAndMemoryHeadless(
        mContext, mDevice, extent, mSwapChainBundle.colorFormat);
    mMemory.insert({image, memory});
    handles[i] = handle;
    images[i] = image;
  }
  ...
  mSwapChainBundle.depthFormat = selectDepthFormat(
      mContext.getAttachmentDepthStencilFormats(), hasStencil);
...
VkResult VulkanPlatformHeadlessSwapChain::present(uint32_t index,
                                                  VkSemaphore finished) {
...
void VulkanPlatformHeadlessSwapChain::destroy() {
  // This is only ever called from the destructor since headless does not
  // recreate.
  ...
  mSwapChainBundle.colors.clear();
  // No need to manually call through to the super because the super's
